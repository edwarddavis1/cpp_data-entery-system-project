...
experiment::experiment(std::string experiment_filename)
{
	if (experiment_filename.find(".txt") != std::string::npos) {
		...
		for (size_t i{ 0 }; i < first_line.length(); i++) {
			if (first_line[i] == ',') number_of_variables++;
		}
		...
		std::vector<std::string> variable_names;
		std::vector<std::vector<std::string>> all_string_data(number_of_variables - datestamp_occurrences - error_occurrences);
		...
		if (all_string_data[0].size() > 0)	number_of_rows = static_cast<int>(all_string_data[0].size());
		else if (all_error_data[0].size() > 0)	number_of_rows = static_cast<int>(all_error_data[0].size());
		else if (all_datestamp_data[0].size() > 0)	number_of_rows = static_cast<int>(all_datestamp_data[0].size());
		...
		for (size_t i{ 0 }; i < string_variable_types.size(); i++) {
			if (string_variable_types[i] == "string") {
				variable_types.push_back(data_variable_types[index]);
				index++;
			} else variable_types.push_back(string_variable_types[i]);
		}
		...
		if (datestamp_occurrences > 0) {
			...
			experiment_datestamps = all_datestamp_data[index];
		} else {
			// Otherwise set as blank dates
			for (int i{ 0 }; i < number_of_rows; i++) experiment_datestamps.push_back("00/00/0000");
		}
		// Add experiment dates
		experiment_data.emplace_back(new nominal_data("Datestamps", experiment_datestamps, experiment_datestamps));
		// Create variables for experiment data
		int index_correction{ 0 };
		for (size_t i{ 0 }; i < variable_types.size(); i++) {
			std::string type{ variable_types[i] };
			if (type == "double") {
				...
				if (!found_errors) {
					for (int i{ 0 }; i < number_of_rows; i++) variable_errors.push_back(0);
				}
				if (!found_datestamps) {
					variable_datestamps = experiment_datestamps;
				}
				// Create variable and add to experiment
				experiment_data.emplace_back(new double_data(variable_names[i], variable_data, variable_errors, variable_datestamps));
				if (found_errors) experiment_data.emplace_back(new double_data(variable_names[i] + " Error", variable_errors, variable_datestamps));
				...
				// Create variable and add to experiment
				experiment_data.emplace_back(new nominal_data(variable_names[i], variable_data, variable_datestamps));
